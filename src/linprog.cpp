...
public:
    ...
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs)
    {
        ...
        if (inputs.size() < 3)
            utilities::error("At least 3 inputs are required: f, A, b.");

        matlab::data::TypedArray<double> f = std::move(inputs[0]);
        matlab::data::Array Amat = std::move(inputs[1]);
        matlab::data::TypedArray<double> b = std::move(inputs[2]);
        ...
        if (inputs.size() > 3)
        {
            matlab::data::TypedArray<double> bnds = std::move(inputs[3]);
